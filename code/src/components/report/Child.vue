...
export default {
    ...
    mounted() {
        if(this.jsondata.report != null){
            if(JSON.parse(JSON.stringify(Object.keys(this.jsondata.report).length)) < 5){
                this.jsonlength = JSON.parse(JSON.stringify(Object.keys(this.jsondata.report).length))
            }else{
                this.jsonlength = 5
            }
            this.maxlength = JSON.parse(JSON.stringify(Object.keys(this.jsondata.report).length))
            this.jsontab = Object.values(this.jsondata.report)
            this.pagelength = Math.floor(this.maxlength/5)+1
        }
    },
    watch: {
        jsondata:function(one){
            if(JSON.parse(JSON.stringify(Object.keys(one.report).length)) < 5){
                this.jsonlength = JSON.parse(JSON.stringify(Object.keys(one.report).length))
            }else{
                this.jsonlength = 5
            }
            this.jsontab = Object.values(one.report)
            this.maxlength = JSON.parse(JSON.stringify(Object.keys(one.report).length))
            this.pagelength = Math.floor(this.maxlength/5)+1
